...
% choosing option (c) because it is the sigmoid function equation applied to
% the inputs to the hidden units.
...
% choosing option (a) because expected dimensionality of inputs_to_softmax
% is (vocab_size * batchsize). 
% The first term `hid_to_output_weights' * hidden_layer_state` yields a matrix
% with dim (vocab_size * batchsize).
% The second term repmat(output_bias, 1, batchsize) yields a matrix with dim
% (1, batchsize)
